...
int main()
{
	...
	//const char* path = "./dataset/Youtube";
	...
	//Exact
	std::vector<std::string> ids = {"263","284","489","492","498","462","504","560","261","1478"};
	//std::vector<std::string> ids = {"134","461","2234","2386","3870","4264","6681","9874","11870","14439"};
	...
	for(int i=0;i<10;i+=2){
		...
		//清除SGX中的CF
		ecall_clear_CFs(eid);
		std::cout << "rebuilt CF Total time: " <<load_CF_total_time<< " ms" << std::endl;

		std::cout << "Final Search time: " <<end_time - start_time - load_CF_total_time<< " ms" << std::endl;
	...
	std::string searchinput = "HelenMo";
	start_time =  timeSinceEpochMillisec();
	ecall_Conjunctive_Fuzzy_Social_Search(eid,(char*)searchinput.c_str());
	end_time =  timeSinceEpochMillisec();
	std::cout << "Final Fuzzy Search time: " <<end_time - start_time - load_CF_total_time<< " ms" << std::endl;
	load_CF_total_time = 0;
	...
	/*******************删除******************/
	// uint64_t delete_start_time =  timeSinceEpochMillisec();
	// // 逐行读取文件
	// lineNumber = 0;
	// file.open(path);
    // while (std::getline(file, line)) {
	...
	// 	const char* id = sid.c_str();
	// 	size_t id_len = sid.length();
	// 	ecall_update_data(eid,w, w_len, id, id_len, 0);
	// 	lineNumber++;
    // }

	// uint64_t delete_end_time =  timeSinceEpochMillisec();
	// file.close();

	// std::cout << "Delete Total Time: " <<delete_end_time - delete_start_time<< " ms" << std::endl;
