...
void ecall_init(unsigned char key_array[3][16]){
    ...
    MostCFs = 40*1024*1024*0.9*0.18 / (single_table_length*byteperbucket);
    ...
    for(int j=0;j<CidListSize;j++){
        ...
        for(int i=0;i<tokens.size();i++){
            if(i != LeastWIndex){
                ...
                generateIF(tokens[i].c_str(),sid.c_str(),index,fingerprint,fingerprint_size,single_table_length);
